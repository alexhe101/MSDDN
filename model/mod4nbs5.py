...
class CGConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,bias=True,g_inchannels=3):  #
        ...
        self.conv1=ConvBlock(g_inchannels, inter_channels, 1, 1, 0, activation='prelu', norm=None, bias = False)
        self.conv3=ConvBlock(g_inchannels, inter_channels, 3, 1, 1, activation='prelu', norm=None, bias = False)
        self.conv5=ConvBlock(g_inchannels, inter_channels, 5, 1, 2, activation='prelu', norm=None, bias = False)
        self.convm1=ConvBlock(inter_channels*3, inter_channels*3, 1, 1, 0, activation='prelu', norm=None, bias = bias)
        self.convm2=nn.Conv2d(in_channels=inter_channels*3, out_channels=inter_channels*2, kernel_size=3, stride=1, padding=1,bias=bias)
    ...
    def forward(self,input,g_input):
        x1=self.conv1(g_input)
        x3 = self.conv3(g_input)
        x5 = self.conv5(g_input)
        x=torch.cat((x1,x3,x5),dim=1)
        #print(x.shape)
        x=self.convm1(x)
        #print(x.shape)
        x=self.convm2(x)
        scale, translation = torch.split(x, (self.split,self.split), dim=1)
