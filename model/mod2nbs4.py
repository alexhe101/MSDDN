...
class FoldConv_aspp(nn.Module):
    def __init__(self, in_channel, out_channel, out_size=24,
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(fold_C, down_dim, kernel_size, stride, padding, dilation, groups,bias=False),
            #nn.BatchNorm2d(down_dim),
            nn.PReLU()
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(fold_C, down_dim, kernel_size=3, dilation=2, padding=2,bias=False), 
            #nn.BatchNorm2d(down_dim), 
            nn.PReLU()
        )
        self.conv4 = nn.Sequential(
            nn.Conv2d(fold_C, down_dim, kernel_size=3, dilation=3, padding=3,bias=False),
            #nn.BatchNorm2d( down_dim), 
            nn.PReLU()
        )
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(4 * down_dim, fold_C, kernel_size=1,bias=False),  #down_dim=48
    ...
    def forward(self, in_feature):
        ...
        #print(in_feature5.shape)
        #print(in_feature1.shape)
        #print(in_feature2.shape)
        in_feature = self.fuse(torch.cat((in_feature1,in_feature2, in_feature3,in_feature4), 1)) #in_feature1,
...
class CGConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=True):  #
        ...
        setattr(self, 'fc{}', nn.Sequential(*[
            nn.Conv2d(in_channels=3, out_channels=inter_channels, 
...
class Bandmodulation(nn.Module):
    def __init__(self,in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=True,split=4):
        ...
        for i in range(self.num_split):
            setattr(self, 'conv{}'.format(i), nn.Conv2d(in_channels=in_split+3, out_channels=in_split, 
                                                        kernel_size=kernel_size, stride=stride, padding=padding, bias=bias))
    ...
    def forward(self, input,g_input):
        ...
        output = [] 
        g_input=torch.cat((g_input,g_input,g_input),1)
        for i in range(self.num_split):
            split_fusion=getattr(self,'conv{}'.format(i))(torch.cat((t_input[i],g_input),1))
...
class ab_DualGCN_Spatial_fist(nn.Module):
    def __init__(self, inchannels):
        ...
        self.BMM=Bandmodulation(inchannels,inchannels)
        self.sGCN = SpatialGCN(inchannels)
        self.FoldConv_aspp=FoldConv_aspp(inchannels,inchannels,kernel_size=3,stride=1,padding=2,dilation=2,win_size=2,win_padding=0)
        self.conv_1 = nn.Sequential(
            nn.Conv2d(inchannels, inchannels, 3, padding=1, dilation=1),
            nn.ReLU(inchannels)
        )
