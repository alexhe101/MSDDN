...
class FoldConv_aspp(nn.Module):
    def __init__(self, in_channel, out_channel,out_size=128,
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(fold_C, down_dim, kernel_size=3,padding=2, dilation=2,bias=False),
            #nn.BatchNorm2d(down_dim),
            nn.PReLU()
        )
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(in_channel+3*cat_dim, out_channel, kernel_size=1,bias=False),  #down_dim=48    fold_C
    ...
    def forward(self, in_feature):
        ...
        in_feature = self.fuse(torch.cat((in_feature0, in_feature2, in_feature3,in_feature4), 1)) #in_feature1,
