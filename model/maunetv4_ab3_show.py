...
class U_block(nn.Module):
    def __init__(self,in_channel=4,mid_channel=4):
        ...
        #self.conv1 =C_RB(in_channel+1,mid_channel,5,1,2,bias=False,activation='prelu',norm=None,n_resblocks=1)
        #self.conv2=ConvBlock(mid_channel,in_channel, 5, 1, 2, activation='prelu', norm=None, bias = False)
    ...
    def forward(self,input,pan):
        ...
        pan_f=torch.cat((pan_f,pan),dim=1) #4
        x=torch.cat((input,pan_f),dim=1)
        x=self.conv1(x)
        U=self.conv2(x)
        mem_f=torch.cat((U,x,input),dim=1)#12
        return U,mem_f #4,12
class Hl_block(nn.Module):
    def __init__(self,in_channel=4,mid_channel=4,o_channel=4):
        ...
        self.upconv=nn.ConvTranspose2d(mid_channel,o_channel,5,4,1,1,bias=False)  #转置卷积 (in-1)*stride+outpad-2*pad+kernelsize
...
class vanilla_stage(nn.Module):
    def __init__(self):
        ...
        self.H2SH_outup=nn.ConvTranspose2d(4,4,3,2,1,1,bias=False)  #转置卷积 (in-1)*stride+outpad-2*pad+kernelsize

    def forward(self,H,L,pan):
        U,U_mem_out=self.U(H,pan) #4,12
...
class Net(nn.Module):
    def __init__(self, base_filter=None, args=None,num_channels=4,stage=4):
        super(Net,self).__init__()
        self.stage_num=stage
        n_resblocks=1
        #self.input_conv=ConvBlock(num_channels, num_channels*2, 9, 1, 4, activation='prelu', norm=None, bias = False)

        self.stage0=vanilla_stage()
        
        ###记忆支路
        self.Uconv1=C_RB(12,4,3,1,1,bias=False,activation='prelu',norm=None,n_resblocks=2)
        self.cellU=ConvLSTMCell((128,128),4,4,[3,3],False)
        self.Uconv2=C_RB(4,8,3,1,1,bias=False,activation='prelu',norm=None,n_resblocks=2)

        self.Hlconv1=C_RB(12,8,3,1,1,bias=False,activation='prelu',norm=None,n_resblocks=2)
        self.cellHl=ConvLSTMCell((128,128),8,4,[3,3],False)
        self.Hlconv2=C_RB(4,4,3,1,1,bias=False,activation='prelu',norm=None,n_resblocks=2)

        self.Vconv1=C_RB(12,4,3,1,1,bias=False,activation='prelu',norm=None,n_resblocks=2)
        self.cellV=ConvLSTMCell((128,128),4,4,[3,3],False)
        self.Vconv2=C_RB(4,8,3,1,1,bias=False,activation='prelu',norm=None,n_resblocks=2)

        self.Hpconv1=C_RB(12,8,3,1,1,bias=False,activation='prelu',norm=None,n_resblocks=2)
        self.cellHp=ConvLSTMCell((128,128),8,4,[3,3],False)
        self.Hpconv2=C_RB(4,4,3,1,1,bias=False,activation='prelu',norm=None,n_resblocks=2)

        ##主路
        #self.Uconv3=ConvBlock(12,4,3,1,1,False,'prelu',None)
        self.Uconv3 =C_RB(12, 4, 3, 1, 1, bias=False, activation='prelu', norm=None, n_resblocks=2)
        #self.H2SH=ConvBlock(4,4,3, 1, 1, activation='prelu', norm=None, bias = False)
        #self.H2SH = RB(4, 4, 3, 1, 1, bias=False, activation='prelu', norm=None, n_resblocks=2)
        ...
        self.H2SH_out=C_RB(8, 4, 1, 1, 0, bias=False, activation='prelu', norm=None, n_resblocks=1)
        self.H2SH_outup=nn.ConvTranspose2d(4,4,3,2,1,1,bias=False)  #转置卷积 (in-1)*stride+outpad-2*pad+kernelsize
        #self.Vconv3=ConvBlock(12,4,3,1,1,False,'prelu',None)
    ...
    def forward(self,l_ms,bms,pan):
        ...
        #print("tage0,Hl_mem_f:",Hl_mem_f.shape)
        # time.time() # 返回的是当前的时间戳; 如果想取一个整数则
        show_cnt =int(time.time())
        print(show_cnt)
        for i in range(self.stage_num):
            ...
            U_mem_f=self.Uconv1(U_mem_f)#12->8
            ...
            ###主路###
            H_f=torch.cat((SH,U_mem_f),dim=1) #4+8=12
            H_f=self.Uconv3(H_f)#12->4
            U,U_mem_f=self.U_block(H_f,pan)


            show_img=U.permute(1, 0, 2, 3)
            img_grid_real = torchvision.utils.make_grid(show_img[:32], normalize=True)
            # print(img_grid_real.shape)
            # 保存图片
            vutils.save_image(img_grid_real, './temp_show/U/'+str(show_cnt)+'_'+str(i) + '.png')

            show_img = U.clone().detach().cpu()
            show_img= show_img.numpy().astype(np.uint8)
            show_img=cv2.imread('./temp_show/U/'+str(show_cnt)+'_'+str(i) + '.png')
            im_color = cv2.applyColorMap(show_img, cv2.COLORMAP_JET)
            cv2.imwrite('./temp_show/PU/'+str(show_cnt)+'_'+str(i) + '.png',im_color)


            H,Hl_mem_f=self.Hl_block(SH,U,l_ms,Hl_mem_f)#4,12
            ...
            SH = self.H2SH(SH, pan_in)
            SH = self.H2SH_out(SH)
            SH = self.H2SH_outup(SH)+H  #V4_2
            show_img=SH.permute(1, 0, 2, 3)
            img_grid_real = torchvision.utils.make_grid(show_img[:32], normalize=True)
            # 保存图片
            vutils.save_image(img_grid_real, './temp_show/SH/'+str(show_cnt)+'_'+str(i) + '.png')
            show_img=cv2.imread('./temp_show/SH/'+str(show_cnt)+'_'+str(i) + '.png')
            im_color = cv2.applyColorMap(show_img, cv2.COLORMAP_JET)
            cv2.imwrite('./temp_show/PSH/'+str(show_cnt)+'_'+str(i) + '.png',im_color)


            H_f=torch.cat((SH,V_mem_f),dim=1) #4+8=12
            H_f=self.Vconv3(H_f)#12->4
            V,V_mem_f=self.V_block(H_f,pan)

            # print(V.shape)
            show_img=V.permute(1, 0, 2, 3)
            img_grid_real = torchvision.utils.make_grid(show_img[:32], normalize=True)
            # 保存图片
            vutils.save_image(img_grid_real, './temp_show/V/'+str(show_cnt)+'_'+str(i) + '.png')
            show_img=cv2.imread('./temp_show/V/'+str(show_cnt)+'_'+str(i) + '.png')
            im_color = cv2.applyColorMap(show_img, cv2.COLORMAP_JET)
            cv2.imwrite('./temp_show/PV/'+str(show_cnt)+'_'+str(i) + '.png',im_color)
...
if __name__ == "__main__":
    ...
    print(device)
    model=ConvLSTMCell((128,128),8,8,[3,3],True)  #, map_location=torch.device('cpu')
    ...
    print("o_h,o_c:",o_h.shape,o_c.shape)
