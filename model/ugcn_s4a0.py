...
class ab_DualGCN_Spatial_fist(nn.Module):
    def __init__(self, inchannels):
        ...
        res_block_s1 = [
            ConvBlock(inchannels, 24, 3, 1, 1, activation='prelu', norm='batch', bias = False), #None
        ]
        for i in range(n_resblocks):
            res_block_s1.append(ResnetBlock(24, 3, 1, 1, 0.1, activation='prelu', norm=None))
        res_block_s1.append( ConvBlock(inchannels, 24, 3, 1, 1, activation='prelu', norm='batch', bias = False),)
        self.res_block_s1 = nn.Sequential(*res_block_s1)

        res_block_s2 = [
            ConvBlock(inchannels, 24, 3, 1, 1, activation='prelu', norm='batch', bias = False),
        ]
        for i in range(n_resblocks):
            res_block_s2.append(ResnetBlock(24, 3, 1, 1, 0.1, activation='prelu', norm=None))#None
        res_block_s2.append( ConvBlock(inchannels, 24, 3, 1, 1, activation='prelu', norm='batch', bias = False),)
        ...
        res_block_s3.append( ConvBlock(inchannels, 24, 3, 1, 1, activation='prelu', norm='batch', bias = False),)
