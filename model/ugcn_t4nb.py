...
class Net(nn.Module):
    def __init__(self, num_channels, base_filter, args): 
        ...
        self.encoder_head=ConvBlock(num_channels, interplanes, 9, 1, 4, activation='prelu', norm=None, bias = False)
        self.encoder_gcn_basic=fu_DualGCN_Spatial_fist(interplanes)
        self.encoder_output=ConvBlock(interplanes, num_channels, 5, 1, 2, activation='prelu', norm=None, bias = False)
    ...
    def forward(self,ms,bms,pan):
        #encoder
        ms= self.encoder_head(ms)
        ms= self.encoder_gcn_basic(ms)
        ebms=self.encoder_output(ms)
        # decoder
        pan0=self.head2(pan)
        pan0=torch.cat((pan, pan0), 1) #128  c=4
        cbms=torch.cat((ebms, pan0), 1) #128 c=8
