    ...
    def forward(self,l_ms,bms,pan):
        ...
        for i in range(self.stage_num):
            ...
            if i !=0:
                ...
                Hp_mem_f=self.Hlconv1(Hp_mem_f)
            U_mem_f=self.Uconv1(U_mem_f)#12->8
            V_mem_f=self.Uconv1(V_mem_f)

            u_h,u_c=self.cellU(U_mem_f,cur_state=[u_h, u_c])
            Hl_h,Hl_c=self.cellHl(Hl_mem_f,cur_state=[Hl_h, Hl_c])
            v_h,v_c=self.cellU(V_mem_f,cur_state=[v_h, v_c])
            Hp_h,Hp_c=self.cellHl(Hp_mem_f,cur_state=[Hp_h, Hp_c])

            U_mem_f=self.Uconv2(u_h)  
            Hl_mem_f=self.Hlconv2(Hl_h)
            V_mem_f=self.Uconv2(v_h)
            Hp_mem_f=self.Hlconv2(Hp_h)
