...
class C_RB(nn.Module):
    ...
    bias=True, activation='prelu', norm=None,n_resblocks=2):
        ...
        for i in range(n_resblocks):
            res_block.append(ResnetBlock(output_channel, kernel_size, stride, padding, 0.1, activation=activation, norm=norm))
...
class U_block(nn.Module):
    def __init__(self,in_channel=4,mid_channel=4):
        ...
        self.conv1=ConvBlock(in_channel+1,mid_channel, 5, 1, 2, activation='prelu', norm=None, bias = False)
        self.conv2=ConvBlock(mid_channel,in_channel, 5, 1, 2, activation='prelu', norm=None, bias = False)
    def forward(self,input,pan):
        x=torch.cat((input,pan),dim=1)
        x=self.conv1(x)
        U=self.conv2(x)
...
class Net(nn.Module):
    def __init__(self, base_filter, args,num_channels=4,stage=3):
        ...
        self.cellU=ConvLSTMCell((128,128),8,8,[3,3],True)
        ...
        self.cellHl=ConvLSTMCell((128,128),8,8,[3,3],True)
        ...
        self.cellV=ConvLSTMCell((128,128),8,8,[3,3],True)
        ...
        self.cellHp=ConvLSTMCell((128,128),8,8,[3,3],True)
