...
class Net(nn.Module):
    def __init__(self, base_filter=None, args=None,num_channels=4,stage=3):
        ...
        ##主路
        self.Uconv3,self.H2SH_Hin,self.H2SH_PANin,self.H2SH,self.H2SH_out,self.Vconv3 =nn.ModuleList(),nn.ModuleList(),nn.ModuleList(),nn.ModuleList(),nn.ModuleList(),nn.ModuleList()
        self.U_block,self.Hl_block,self.V_block,self.Hp_block,self.H2SH_outup=nn.ModuleList(),nn.ModuleList(),nn.ModuleList(),nn.ModuleList(),nn.ModuleList()

        for i in range(self.stage_num):
            self.Uconv3.append(C_RB(12, 4, 3, 1, 1, bias=False, activation='prelu', norm=None, n_resblocks=2)) 
            self.H2SH_Hin.append(ConvBlock(4,4,3, 2, 1, activation='prelu', norm=None, bias = False))
            self.H2SH_PANin.append(ConvBlock(1,4,3, 2, 1, activation='prelu', norm=None, bias = False))
            self.H2SH.append(NONLocalBlock2D(in_channels=4, mode='embedded_gaussian'))
            self.H2SH_out.append(C_RB(8, 4, 1, 1, 0, bias=False, activation='prelu', norm=None, n_resblocks=1))
            self.H2SH_outup.append(nn.ConvTranspose2d(4,4,3,2,1,1,bias=False)  )
            self.Vconv3.append(C_RB(12, 4, 3, 1, 1, bias=False, activation='prelu', norm=None, n_resblocks=2))
            
            self.U_block.append(U_block(in_channel=4,mid_channel=4))
            self.Hl_block.append(Hl_block(in_channel=8,mid_channel=4,o_channel=4))
            self.V_block.append(V_block(in_channel=4,mid_channel=4))
            self.Hp_block.append(Hp_block(in_channel=8,mid_channel=4,o_channel=4))
    ...
    def forward(self,l_ms,bms,pan):
        ...
        for i in range(self.stage_num):
            ...
            H_f=self.Uconv3[i](H_f)#12->4
            U,U_mem_f=self.U_block[i](H_f,pan)

            H,Hl_mem_f=self.Hl_block[i](SH,U,l_ms,Hl_mem_f)#4,12

            #SH=self.H2SH(H)
            SH = self.H2SH_Hin[i](H)
            pan_in = self.H2SH_PANin[i](pan)
            SH = self.H2SH[i](SH, pan_in)
            SH = self.H2SH_out[i](SH)
            SH = self.H2SH_outup[i](SH)+H  #V4_2

            H_f=torch.cat((SH,V_mem_f),dim=1) #4+8=12
            H_f=self.Vconv3[i](H_f)#12->4
            V,V_mem_f=self.V_block[i](H_f,pan)

            SH,Hp_mem_f=self.Hp_block[i](SH,V,l_ms,Hp_mem_f)
