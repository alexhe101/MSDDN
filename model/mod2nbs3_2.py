...
class ResnetBlock(nn.Module):
    def __init__(self, input_size, kernel_size=3, stride=1, padding=1, bias=True, scale=1, activation='prelu', norm='batch', pad_model=None):
        ...
        layers = filter(lambda x: x is not None, [self.pad, self.conv1, self.normlayer, self.act, self.pad,self.conv2,self.normlayer,self.act])#, self.conv2, self.normlayer, self.act
...
class ab_DualGCN_Spatial_fist(nn.Module):
    def __init__(self, inchannels):
        ...
        res_block_s3 = [
            ...
            ResnetBlock(24, 3, 1, 1, 0.1, activation='prelu', norm=None)
        ]
        for i in range(n_resblocks):
    ...
    def forward(self, x,g_x):
        ...
        #F_DCM = self.conv_5(torch.cat([F_sGCN, conv1, conv2, conv3, conv4], dim=1))
        F_DCM=self.res_block_s2(F_sGCN)
        #F_cGCN=F_DCM
        #F_cGCN = self.cGCN(F_DCM)
        F_cGCN = self.res_block_s3(F_DCM)
