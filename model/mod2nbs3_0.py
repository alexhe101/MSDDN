...
class Bandmodulation(nn.Module):
    def __init__(self,in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=True,split=4,copy_wight=3,g_inchannels=1):
        ...
        for i in range(self.num_split):
            setattr(self, 'conv{}'.format(i), nn.Conv2d(in_channels=in_split+copy_wight*g_inchannels, out_channels=in_split, 
                                                        kernel_size=kernel_size, stride=stride, padding=padding, bias=bias))
...
class Net(nn.Module):
    ...
    def forward(self,l_ms,bms,pan):
        ...
        s_x=self.gcn_basic1(x1,bms)#24   pan0
        #s_x=self.gcn_basic0(s_x,pan)
        x=self.gcn_basic2(s_x,bms) #24
