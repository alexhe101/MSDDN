...
class ChannelFuseBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, num_res=1, stride=1, downsample=None):
        ...
        self.conv_up = conv3x3(4, planes, stride)#8
        self.epsilon = 1e-4
...
class HighResolutionModule(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # out=np.array(x_fuse)#[3][1, 32, 128, 128]
        # print(out[0].shape)
...
class Net(nn.Module):
    def __init__(self, num_channels, base_filter, args): #, num_channels, base_filter, args
        super(Net, self).__init__()
        mode = "SUM"
        spectral_num=num_channels
        ...
        num_channels = [self.channels_list[1][i] * BasicBlock.expansion for i in range(len(self.channels_list[1]))]

        # print("num_channels",num_channels)#[32, 64]
        # print("stage1_out_channel",stage1_out_channel)#256

        self.transition1 = self._our_make_transition_layer([stage1_out_channel], num_channels)
        ...
        self.fup1 = freup_pad(128)

        self.fup3 = freup_pad(128)

        self.fup2 = freup_pad(64)
    ...
    def forward(self, l_ms, b_ms, x_pan):  #def forward(self, l_ms, b_ms, x_pan):
        '''
        ...
        x=x_pan
        ly=b_ms
        my = F.interpolate(l_ms, scale_factor=2, mode='bicubic')
        sy=l_ms
        if self.first_fuse == "SUM":
            x = x.repeat(1, 4, 1, 1) #8
            x = x + ly
        ...
        x1s = F.interpolate(x[1], size=(x0_h, x0_w), mode='bilinear', align_corners=True)
        # print(x1s.shape)
        x1 = x1s + self.fup2(x[1])


        x2s = F.interpolate(x[2], size=(x0_h, x0_w), mode='bilinear', align_corners=True)
        # print(x2s.shape)
        # print(self.fup1(x[2]).shape)
        x2 = x2s + self.fup3(self.fup1(x[2]))
